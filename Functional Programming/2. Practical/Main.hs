...
drawMaze :: Maze -> IO()
drawMaze maze = putStr( (foldr f "" [0..(i-1)]) ++ (drawLastline maze) )
                    where f rowNo str = str  ++ (drawH rowNo maze) ++ "\n" ++ drawV rowNo maze ++ "\n"
                          i= snd (sizeOf maze)

drawH :: Int -> Maze -> String
drawH rowNo maze = foldr f "+" [0.. (j-1)]
                where f columnNo str =  if (hasWall maze (j-1-columnNo,rowNo) N) then str ++ "--+" else str ++ "  +" 
                      j = fst (sizeOf maze)




drawV ::  Int ->  Maze -> String
drawV rowNo maze = (foldr f "" [0.. (j-1)]) ++ "|"
                where f columnNo str = if (hasWall maze (j-1-columnNo,rowNo) W) then str ++ "|  " else str ++ "   "
                      j = fst (sizeOf maze)

drawLastline :: Maze -> String
drawLastline maze = foldr f "+" [0.. (j-1)] ++ "\n"
                where f columnNo str = str  ++ "--+"
                      j = fst (sizeOf maze)
...
solveMazeIter maze target startingList = g (foldr f startingList startingList)
                    where f (place, path) lst = lst ++ [((move direction place), (path ++ [direction]) ) | direction <- [N,S,E,W], not(hasWall maze place direction) ]
                          p x | target == fst x =True
                              | otherwise = False 
                          g pathList | elem target ((map fst) pathList) = snd ( head (filter p pathList) )
                                     | otherwise = solveMazeIter maze target pathList


fastSolveMazeIter :: Maze -> Place -> [(Place, Path)] -> [Place] -> Path
fastSolveMazeIter maze target startingList visited = g (foldr f [] startingList) visited
                    where f (place, path) lst = lst ++ [((move direction place), (path ++ [direction]) ) | direction <- [N,S,E,W], not(hasWall maze place direction), not (elem (move direction place) visited) ]
                          p x | target == fst x =True
                              | otherwise = False 

                          g pathList visited | elem target ((map fst) pathList) = snd ( head (filter p pathList) )
                                             | pathList == [] = error("This is an impossible maze")
                                             | otherwise = fastSolveMazeIter maze target pathList (visited ++((map fst) pathList))
...
exampleMazeWalls1 :: Int-> Int -> [Wall]
exampleMazeWalls1 m (-1) = []  
exampleMazeWalls1 m n = (run (2*n,0) (m-2) E) ++ (run (2*n+1,1) (m-2) E) ++ (exampleMazeWalls1 m (n-1))

exampleMaze1:: Int -> Int -> Maze
exampleMaze1 n m = makeMaze (2*n+1,m) (exampleMazeWalls1 (m+1) n)

exampleMazeWalls2 :: Int-> Int -> [Wall]
exampleMazeWalls2 (-1) n = []  
exampleMazeWalls2 m n = (run (0,2*m) (n-2) N) ++ (run (1,2*m+1) (n-2) N) ++ (exampleMazeWalls2 (m-1) n)

exampleMaze2:: Int -> Int -> Maze
exampleMaze2 n m = makeMaze (n,2*m+1) (exampleMazeWalls2 m (n+1))

{-
drawMaze ( exampleMaze1 4 8)
+--+--+--+--+--+--+--+--+--+
|     |     |     |     |  |
+  +  +  +  +  +  +  +  +  +
|  |  |  |  |  |  |  |  |  |
