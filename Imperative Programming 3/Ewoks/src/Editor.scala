...
class Editor {
    ...
    def transposeCommand():Boolean= {
    	...
    	if (ed.point< ed.length && 0< ed.point){
    ...
    def searchCommand(){
    val question = "Which word do you want to search "
    val searchedWord= MiniBuffer.searchWord(display, question)
    val cursorPosition= ed.search(searchedWord,ed.point)
    if (cursorPosition== -1 || cursorPosition==ed.point ){
    beep()
    }
    else{ ed.point = cursorPosition    }
    ...
    def swapMarkandPoint()={
    var temp=ed.point
    ed.point= ed.mark
    ed.mark=temp
    }
    
    def copyCutPaste(x:String)={
    	x match {
    		case "C" => 
    			ed.copy()
    		case "X" => 
...
object Editor {
    ...
    val keymap = Keymap[Command](
        ...
        Display.ctrl('M')      -> (_.setMarktoPoint() ),
        Display.ctrl('O')      -> (_.swapMarkandPoint() ),
        Display.ctrl('C')      -> (_.copyCutPaste("C") ),
        Display.ctrl('X')      -> (_.copyCutPaste("X") ),
        Display.ctrl('V')      -> (_.copyCutPaste("V") ),
